# Mainly inspired from https://github.com/dyywinner/ESIM-pytorch
...
class Seq2SeqEncoder(nn.Module):
    ...
    def forward(self, sequences_batch, sequences_lengths):
        ...
        sorted_lengths = sorted_lengths.to(torch.device("cpu"))
...
class SoftmaxAttention(nn.Module):
    ...
    def forward(self,
                ...
                premise_mask,
                hypothesis_batch,
                hypothesis_mask, outlogit=False):
        """
        Args:
        ...
        attended_hypotheses = weighted_sum(premise_batch,
                                           ...
                                           hypothesis_mask)
        if outlogit:
            return attended_premises, attended_hypotheses, torch.log(hyp_prem_attn+1e-20)
        else:
...
class BiasedAttention(nn.Module):
    ...
    def __init__(self):
        ...
        torch.nn.init.uniform_(self.merge.weight)
    def biased_matrix(self, similarity_matrix, bias_matrix):
        matrix = torch.stack([similarity_matrix, bias_matrix], dim=-1).float()
    ...
    def forward(self,
                ...
                premise_mask,
                hypothesis_batch,
                hypothesis_mask, bias_matrix, outlogit=False):
        """
        Args:
            premise_batch: A batch of sequences of vectors representing the
                premises in some NLI task. The batch is assumed to have the
        ...
        similarity_matrix = premise_batch.bmm(hypothesis_batch.transpose(2, 1)
                                                              .contiguous())
        _, l1, l2 = similarity_matrix.size()
        bias_matrix = bias_matrix[:,:l1,:l2]
        matrix = self.biased_matrix(similarity_matrix, bias_matrix)
        
        # Softmax attention weights.
        prem_hyp_attn = masked_softmax(matrix, hypothesis_mask)
        # matrix = self.biased_matrix(prem_hyp_attn, bias_matrix)
        # prem_hyp_attn = masked_softmax(matrix, hypothesis_mask)
        hyp_prem_attn = masked_softmax(matrix.transpose(1, 2)
                                                        .contiguous(),
                                       premise_mask)
        ...
        attended_hypotheses = weighted_sum(premise_batch,
                                           ...
                                           hypothesis_mask)
        if outlogit:
            return attended_premises, attended_hypotheses, prem_hyp_attn# torch.log(prem_hyp_attn+1e-20)
        else:
...
if __name__ == '__main__':
    ba = BiasedAttention()
    ba.biased_matrix(torch.ones([2,2,2]), torch.ones([2,2,2]))