...
if __name__ == '__main__':
    ...
    f = open(join(result_path, "IRLabel.txt"), 'r', encoding="utf-8")
    contentList = f.read().rstrip("\n").split("\n")
    f.close()
    num_all = len(contentList)
    tt = 0
    ...
    f = open(join(result_path, "ATLASLabel.txt"), 'r', encoding="utf-8")
    contentAssertLabelATLAS = f.read().rstrip("\n").split("\n")
    f.close()

    f = open(join(result_path, "ATLASPre.txt"), 'r', encoding="utf-8")
    ...
    f = open(join(result_path, "IR.txt"), 'r', encoding="utf-8")
    ...
    f = open(join(result_path, "RAadapt-H.txt"), 'r', encoding="utf-8")
    ...
    f = open(join(result_path, "RAadapt-NN.txt"), 'r', encoding="utf-8")
    ...
    f = open(join(result_path, "CombineLearningIndex.txt"), 'r', encoding="utf-8")
    contentIndex = f.read().rstrip("\n").split("\n")
    f.close()

    f = open(join(result_path, "Combine.txt"), 'r', encoding="utf-8")
    ...
    print("tot" + "  & " + str(tot1) + "  & " + str(tot2) + "  & " + str(tot3) + "  & " + str(tot4) + "  & " + str(tot6) + "  & " + str(tot7) + "\\\\")

    print("tot" + "  & " + str(round(tot1 / num_all, 2)) + "  & " + str(round(tot2 / num_all, 2)) + "  & " + str(
        round(tot3 / num_all, 2)) + "  & " + str(round(tot4 / num_all, 2)) + "  & " + "  & " + str(round(tot6 / num_all, 2)) + "  & " + str(round(tot7 / num_all, 2)) + "\\\\")
    ...
    for l in record_TypeTot.keys():
        print(str(record_TypeTot[l]) + "(" + str(round(record_TypeTot[l] / num_all, 2)) + ")", end="")
        print(" &", end="")
